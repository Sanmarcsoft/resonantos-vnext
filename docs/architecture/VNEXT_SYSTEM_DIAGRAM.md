...
| Provider fabric | Central route resolution, provider profiles, runtime nodes, diagnostics, smoke tests, MiniMax path, local runtime representation. | Cost-aware strategy is recognized but not fully exposed in UI. | Fallback ladder editor, provider health history, OpenAI/Anthropic/Gemini/local runtime completion. |
...
5. Add provider strategy UI for cost, fallback ladders, health history, and per-agent model policies.
